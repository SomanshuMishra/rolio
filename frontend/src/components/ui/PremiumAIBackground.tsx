...
export default function PremiumAIBackground() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Renderer with better defaults for performance
      ...
      // Mouse Move Handler - throttled to reduce state updates
      let lastMouseTime = 0
      const handleMouseMove = (e: MouseEvent) => {
        const now = Date.now()
        if (now - lastMouseTime > 16) {
          lastMouseTime = now
          cameraTarget.current.x = (e.clientX / window.innerWidth) * 2 - 1
          cameraTarget.current.y = -(e.clientY / window.innerHeight) * 2 + 1
          cameraTarget.current.x *= 0.5
          cameraTarget.current.y *= 0.5
        }
      }
      ...
      const animate = () => {
        ...
        // Skip frames on lower-end devices (render every other frame on mobile)
